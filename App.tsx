...
const App: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {view === 'customers' && (appMode === 'admin' || appMode === 'collector') && <CustomerList customers={customers} packages={packages} bills={bills} onAdd={addCustomer} onUpdate={updateCustomer} onBulkStatusUpdate={async (ids, s) => { await Promise.all(ids.map(id => api.update('customers', id, { status: s }))); await syncData(); }} onDelete={id => { api.delete('customers', id); syncData(); }} isCollector={appMode === 'collector'} />}
      ...
      {view === 'billing' && <BillingList bills={bills} customers={customers} collectors={collectors} packages={packages} onGenerate={generateBills} onMarkPaid={markAsPaid} onRejectPayment={async id => { await api.update('bills', id, { status: BillStatus.UNPAID, paymentMethod: '' }); await syncData(); }} onMarkMultiplePaid={markMultiplePaid} onDelete={deleteBill} onDeleteMultiple={deleteMultipleBills} onAssignCollector={assignBillsToCollector} isCollector={appMode === 'collector'} collectorId={activeCollector?.id} businessName={adminProfile.businessName} />}
      ...
      {view === 'settings' && (
        appMode === 'admin' 
        ? <AdminSettings adminProfile={adminProfile} onUpdate={async u => { await api.updateAdmin(u); await syncData(); }} />
        : activeCollector ? <CollectorProfile collector={activeCollector} onUpdate={async (id, u) => { await api.update('collectors', id, u); await syncData(); }} /> : null
      )}
    </Layout>
  );
};
