...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  // Mobile: default false (closed). Desktop: default true (open).
  const [isSidebarOpen, setSidebarOpen] = useState(false);
  
  // Detect screen size to set initial sidebar state
  useEffect(() => {
    const handleResize = () => {
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      {/* Sidebar */}
      <aside 
        className={`bg-indigo-900 text-white w-64 fixed inset-y-0 left-0 transform transition-transform duration-300 ease-in-out z-50 
      ...
      >
        ...
        <nav className="mt-2 px-4 h-[calc(100vh-100px)] overflow-y-auto">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => handleNavClick(item.id)}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-2 transition-all ${currentView === item.id ? 'bg-indigo-700 text-white shadow-lg' : 'text-indigo-200 hover:bg-indigo-800'}`}
            >
          ...
          <div className="my-6 border-t border-indigo-800 pt-6">
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-2 bg-amber-600 hover:bg-amber-700 text-white shadow-md transition-all font-bold"
            >
              <svg className="w-5 h-5 min-w-[1.25rem]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
              <span className="truncate">Portal Pelanggan</span>
            </button>
            <button
              onClick={onLogout}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-lg mb-2 text-indigo-300 hover:bg-rose-600 hover:text-white transition-all"
            >
        ...
        <header className="bg-white dark:bg-slate-900 dark:border-slate-800 border-b sticky top-0 z-30 px-4 md:px-6 py-4 flex items-center justify-between shadow-sm transition-colors duration-300">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-2 p-2 px-3 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all font-bold text-xs"
            >
              {isDark ? (
                <>
          ...
          <div className="flex items-center gap-3">
             ...
             <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center text-indigo-600 dark:text-indigo-300 font-black border-2 border-indigo-50 dark:border-indigo-800 shadow-sm">
