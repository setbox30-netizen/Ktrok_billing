...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  const allNavItems = [
    ...
    { id: 'collectors' as View, label: 'Kang Tagih', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
  ...
  const navItems = role === 'collector' 
    ? allNavItems.filter(item => item.id === 'dashboard' || item.id === 'customers' || item.id === 'billing' || item.id === 'settings') 
    : allNavItems;
      ...
      <aside className={`bg-indigo-900 text-white w-64 fixed inset-y-0 left-0 transform transition-transform duration-300 ease-in-out z-50 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        <div className="p-6 flex items-center justify-between">
          <div>
              ...
              <p className="text-indigo-300 text-[10px] mt-1 uppercase tracking-widest font-black">{role === 'collector' ? 'Collector Panel' : 'Management Panel'}</p>
          </div>
